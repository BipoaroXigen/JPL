...
function RRT(q_init::State, env::Env, K=100000, s=100)
	...
	open("/home/krizjona/Project/files/points.txt", "w") do w
	end
	...
	for r in 1:K
		...
		while true
			if helped < volume
				cen = [clamp(randn()*3+impact[1], env.x_min, env.x_max),
				       clamp(randn()*3+impact[2], env.y_min, env.y_max),
				       clamp(randn()*3+impact[3], env.z_min, env.z_max)]
...
#					q = angle_to_quat(rot[1], rot[2], rot[3], :ZYX)
		...
		if connection == q_new
			if r%5 == 0
			open("/home/krizjona/Project/files/points.txt", "a") do w
				println(w, string(q_new.x), " ", string(q_new.y), " ", string(q_new.z))
			end
			end
