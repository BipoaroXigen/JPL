...
function presolve(q_init::State, env::Env, K=100000, s=100)::TREE
	#Random.seed!(13)
#	tree, nn = init_tree(env, q_init)

	tree = get_tree(q_init.q)
	...
	for r in 1:K
		...
		while true
			...
			cen = [clamp(randn() * 10 + point[1], env.x_min, env.x_max),
			       clamp(randn() * 10 + point[2], env.y_min, env.y_max),
			       clamp(randn() * 10 + point[3], env.z_min, env.z_max),
			       ]
			...
			q_near_node = tree.nodes[knn(tree.flann, q_new.q, 1)[1][1]]
		...
		q_new_node = t_append!(tree, q_near_node, connection.q)
